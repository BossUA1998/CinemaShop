...
class Payment(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    status: Mapped[PaymentStatus] = mapped_column(Enum(PaymentStatus), nullable=True)
    amount: Mapped[Decimal] = mapped_column(Numeric(10, 2), nullable=False)
    external_payment_id: Mapped[str] = mapped_column(String(255), unique=True, nullable=True)
...
class PaymentItem(Base):
    ...
    __table_args__ = (
        UniqueConstraint("payment_id", "order_item_id"),
    )
